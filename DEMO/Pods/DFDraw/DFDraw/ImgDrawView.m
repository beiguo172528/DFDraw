...
- (void)initWithImage: (UIImage *)image withNavigationbarHeight:(NSInteger)navigationbarHeight
{
   ...
   //高度减去状态栏，导航条和按钮的高度
    ...
    screenSize.height -= navbarHeight + 64+ statusBarHeight;
    ...
    int    imageViewLeft   =  0;
    int    imageViewtop    =  navbarHeight+statusBarHeight;
...
-(void)changeFrameForGestureView:(UIView *)view
{
    ...
    if (@available(iOS 13.0, *)) {
        ...
        statusBarHeight = navbarHeight + 64+[[UIApplication sharedApplication] statusBarFrame].size.height;
    }
    //高度减去导航条和按钮的高度
    screenSize.height -= navbarHeight + 64+statusBarHeight;
    
    int blackViewHeight   = (screenSize.height - _oldImageViewFrame.size.height)/2;
    int topOffset         = navbarHeight+statusBarHeight;
