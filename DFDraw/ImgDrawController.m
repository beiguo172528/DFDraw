...
- (instancetype)initNib{
    NSBundle *bundle = [NSBundle bundleWithBundleName:@"DFDraw" podName:@"DFDraw"];
    self = [super initWithNibName:@"ImgDrawController" bundle:bundle];
    return self;
}
...
- (void) setBtnState
{
    ...
    NSBundle *bundle = [NSBundle bundleWithBundleName:@"DFDraw" podName:@"DFDraw"];
    [self.DragBtn setImage: [UIImage imageNamed: [self.imgDrawView getState] == DfDrag ? @"drag_enable" : @"drag_disable" inBundle:bundle compatibleWithTraitCollection:nil] forState: UIControlStateNormal];
    [self.DrawBtn setImage: [UIImage imageNamed: [self.imgDrawView getState] == DfDraw ? @"draw_enable" : @"draw_disable" inBundle:bundle compatibleWithTraitCollection:nil] forState: UIControlStateNormal];
...
- (IBAction)WidthBtnClick:(id)sender
{
    ...
    pencilView.frame = CGRectMake(0, self->_ColorBtn.frame.origin.y + self->_ColorBtn.frame.size.height - 64 , self.view.frame.size.width, 64);
...
- (IBAction)ColorBtnClick:(id)sender
{
    ...
    view.frame = CGRectMake(0, self->_ColorBtn.frame.origin.y + self->_ColorBtn.frame.size.height - 64, self.view.frame.size.width, 64);
...
- (IBAction)stateBtnClicked:(id)sender
{
    ...
    if(DfState == DfDraw)
    {
        [_stateBtn setImage:[UIImage imageNamed:@"draw_drag" inBundle:bundle compatibleWithTraitCollection:nil] forState:UIControlStateNormal];
        [self setDrawEnable:YES];
        [self DrawBtnClick:nil];
    }
    else
    {
        [_stateBtn setImage:[UIImage imageNamed:@"draw_draw" inBundle:bundle compatibleWithTraitCollection:nil] forState:UIControlStateNormal];
